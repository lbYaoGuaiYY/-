...
status: active
intent: updated-to-reflect-reality
pending-action: user review and direction
approach: Keep the working React 19 and Fabric 7 editor as the product base. Tauri is now the primary distribution target, with IndexedDB/browser fallback. The multi-project boundary and Python-based cloud asset library (with local caching) are now implemented and moving to integration.
---
...
| Scope discipline | No AI, templates, or accounts. Multi-project and cloud asset backend are now IN SCOPE and partially implemented | The reality of the current codebase includes ProjectHome, Tauri project storage, and a Python FastAPI asset backend. We are embracing this expanded scope. | yes |
...
| Distribution target | Tauri 2 for Windows desktop is the primary target, browsers are a fallback | Tauri allows native file system access for project storage and asset caching, which we have already built | yes |

## Findings (cited - path:lines)

- DESIGN.md defines a quiet, canvas-first compositor and excludes Photoshop-scale complexity, templates, AI, and login.
...
- The ProjectHome dashboard and Tauri native file dialogs are now implemented, shifting us from single-project to a multi-project workspace.
...
- Embrace the multi-project ProjectHome and Tauri native capabilities that have already been built.
- Embrace the cloud asset backend (Python/FastAPI) and the local/native caching layer that have been built.
...
- **Multi-project dashboard and Tauri local filesystem project storage.**
- **Python/FastAPI cloud asset backend and local offline caching layer.**

## Scope OUT (Must NOT have)

- AI generation or editing.
- Templates, account/login, analytics, or real-time collaboration.
...
- Treat the current 杞昏 application as the primary finished base instead of discarding it: Fabric 7.4 already supplies mature move, scale, rotate, grouping, image IO, and export primitives under MIT.
...
| Deferred work | PWA, real-time collaboration, 3D, AI, and generic design-suite features | Multi-project and packaging are now active. The rest do not determine whether an existing editor can power the focused wedding workflow | yes |
...
The codebase has advanced significantly beyond the initial MVP plan. Tauri packaging, multi-project views, and a cloud asset backend are all implemented and merged. The roadmap has been updated to reflect these new baselines.
