...
test("restores one complete active project after reload", async ({ page }) => {
  ...
  await setImageInput(page, "asset-file-input", "local-cutout.png")
  await expect(page.getByTestId("layer-list")).toContainText("local-cutout")
  await page.getByRole("button", { name: "local-cutout", exact: true }).click()
...
test("rejects a corrupt draft without partially restoring it", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    await new Promise<void>((resolve, reject) => {
      const request = indexedDB.open("qingshe-projects-v1", 1)
